...
namespace Microsoft.AspNetCore.Server.Kestrel.Core.Internal.Http2
{
    ...
    internal class Http2KeepAlive
    {
        ...
        {
            var timestamp = _systemClock.CurrentTicks;
            ...
            if (frameReceived)
            {
                ...
                _lastFrameReceivedTimestamp = timestamp + 1000;
            ...
            else
            {
                switch (_state)
                {
                    case KeepAliveState.None:
                        ...
                        if (timestamp > (_lastFrameReceivedTimestamp + (long)_keepAliveInterval.TotalMilliseconds))
                        {
                            ...
                            // System clock only has 1 second of precision, so the clock could be up to 1 second in the past.
                            // To err on the side of caution, add a second to the clock when calculating the ping sent time.
                            _pingSentTimestamp = _systemClock.CurrentTicks + 1000;
                    ...
                    case KeepAliveState.PingSent:
                        ...
                        {
                            if (timestamp > (_pingSentTimestamp + (long)_keepAliveTimeout.TotalMilliseconds))
                            {
